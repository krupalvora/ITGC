...
def execute(filters=None):
	...
	# ref_doctype is fixed because this report is specific to User changes.
	# The password-pattern excludes are security best-practice — never expose
	# password reset tokens or new_password values in audit output.
	conditions = [
		"ref_doctype = 'User'",
		f"{DATE_FIELDS[date_field]} BETWEEN %(from_date)s AND %(to_date)s",
		"data NOT LIKE '%%reset_password_key%%'",
		"data NOT LIKE '%%new_password%%'",
		"data NOT LIKE '%%last_password_reset_date%%'",
	]
