...
def _set_settings(allow_public, token=None):
	frappe.db.set_single_value(
		"ITGC Settings", "allow_public_manage_change_status_endpoint", 1 if allow_public else 0
	)
...
class TestManageChangeGate(FrappeTestCase):
	...
	fail-closed: token required by default, public only on explicit opt-in.

	Each test sets ITGC Settings via set_single_value/set_encrypted_password
	(avoiding the doctype's on_update side effects) and mocks the request header.
	FrappeTestCase rolls everything back per test.
	"""
	...
	def test_default_is_fail_closed(self):
		"""Not public + no token configured => deny, even with no header."""
		_set_settings(allow_public=False, token="")
		self.assertFalse(self._auth_with_header(None))
		self.assertFalse(self._auth_with_header("anything"))

	def test_protected_requires_correct_token(self):
		_set_settings(allow_public=False, token=TOKEN)
	...
	def test_public_opt_in_skips_token(self):
		_set_settings(allow_public=True, token="")
		self.assertTrue(self._auth_with_header(None))
